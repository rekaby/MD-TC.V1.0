		...
		for (int i = 0; i < len; ++i)
		{
			...
			for (int j = 0; j < len; ++j) 
				{if (i != j && (nopruning || arc2id[j*len+i] != -1)) {
					...
					arcFvs[i*len+j] = pipe.synFactory.createArcFeatures(inst, i, j,usedRelations);
					...
					//System.out.println("i-j "+i+" "+ j+ " "+arcScores[i*len+j]);
				}
				}
		}
...
//	}
		...
		for (int m = 1; m < len; ++m)
			{
				...
				if (ContextConstances.DEBUG) {
	            	System.out.println("i->j "+m+"->"+heads[m]+" = "+arcScores[heads[m]*len+m]);
	            }
			}
		if (options.learningMode != LearningMode.Basic) {
	...
	public FeatureVector getFeatureVector(DependencyInstance now)
	{
		...
		// 1st order arc
		for (int m = 1; m < len; ++m)
			fv.addEntries(arcFvs[heads[m]*len+m]);//coz we have [m][m]arc vectors each one is from each node to another (Combination)
...
//    	//TODO: handle high order features
	...
	{
		List<ContextRelation> usedRelations=new ArrayList<ContextRelation>();
		return pipe.synFactory.createLabelFeatures(inst, arcLis, heads, mod, type,usedRelations);
	}
	
	private double getLabelScore(DependencyArcList arcLis, int[] heads, int mod, int type)
	{
		FeatureVector fVector= getLabelFeature(arcLis, heads, mod, type);
		double result =parameters.dotProductL(fVector) * gammaLabel;
		//System.out.println("%%%%"+mod+ inst.forms[mod]+ "%%%"+heads[mod]+"%%%"+pipe.types[type] +"%%%"+result+"%%%"+ fVector.size());
		return result;
	}
