...
public class Parameters implements Serializable {
	...
	public int size, sizeL;//size of Arc features, SizeL for Label Arc Features
	public int rank;//comes from options, default 50, in main it makes it 0
	public int N, M, T, D;//N number of word features, M = N, D=15??,T is not used
	
	public double[] params, paramsL; //Param list of Double with size of Arc features, paramsL same for label Arc
	public double[][] U, V, W;//U, V Param matrix [rank??][# word features], W Param matrix [rank??][D=15??] 
	...
	public transient FeatureVector[] dU, dV, dW;//array of vector with size of rank, inside each vector there are key/value list
			...
			int updCnt, int offset)
	{
    	...
    	double Fi = getHammingDis(actDeps, actLabs, predDeps, predLabs);//hamming dist of Deps not labels
    	
    	FeatureVector dt = lfd.getFeatureDifference(gold, pred);
    	dt.addEntries(gfd.getFeatureDifference(gold, pred));
    //	System.out.println("In Update the weights, LFD:"+dt.size());    	
    ...
    //Hamming Dist return the diff of two strings, the count of diff elements
