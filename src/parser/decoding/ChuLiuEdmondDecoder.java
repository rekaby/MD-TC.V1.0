...
public class ChuLiuEdmondDecoder extends DependencyDecoder {
	...
	public double calcScore(DependencyInstance now,LocalFeatureData lfd) 
	{
		double score=0;
		 for (int i = 1; i < now.length; ++i) {
	            int j = now.heads[i];
	            //int t = options.learnLabel ? staticTypes[j][i] : 0;
	            //System.out.println(i+"-"+j+":"+lfd.getArcScore(j,i));
	            if (ContextConstances.DEBUG) {
	            	System.out.println("i->j "+i+"->"+j+" = "+lfd.getArcScore(j,i));
	            }
	            score+=lfd.getArcScore(j,i);
	            //predInst.deplbids[i] = t;
	        }
		 if (ContextConstances.DEBUG) {
         //	System.out.println("EXTRA i->j "+"11"+"->"+"7"+" = "+lfd.getArcScore(7,11));
         //	System.out.println("EXTRA i->j "+"11"+"->"+"4"+" = "+lfd.getArcScore(4,11));
         }
		 return score;
	}
	@Override
